...
//! (`MAIL_TEMPLATE_NOT_FOUND` on a missing root template / `MAIL_TEMPLATE_READ_ERROR`)
//! and the loader code (`MAIL_TEMPLATE_NAPI_REQUIRED`); those never originate in
...
//! does not exist), mapped back to the same TS `MAIL_TEMPLATE_NOT_FOUND` code.
//!
...
impl ErrorCode {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            ErrorCode::Syntax => "MAIL_TEMPLATE_SYNTAX",
            ErrorCode::Recursion => "MAIL_TEMPLATE_RECURSION",
            ErrorCode::NotFound => "MAIL_TEMPLATE_NOT_FOUND",
        }
    }
}
...
mod tests {
    ...
    fn code_strings_are_byte_stable() {
        assert_eq!(ErrorCode::Syntax.as_str(), "MAIL_TEMPLATE_SYNTAX");
        assert_eq!(ErrorCode::Recursion.as_str(), "MAIL_TEMPLATE_RECURSION");
        assert_eq!(ErrorCode::NotFound.as_str(), "MAIL_TEMPLATE_NOT_FOUND");
    }
}
